...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
    ...
    // bar's fixed `#0b1512`, which with this lab's `--accent` measures 2.08:1 in
    // dark (`#60a5fa`) and 1.35:1 in light (`#1d4ed8`) — both under 3:1, both
    // reported upward for a fleet-wide pass rather than patched here, where the
    // patch would only diverge this copy from the other 175.
    ...
    for (const el of Array.from(document.querySelectorAll(rootSelector))) {
      ...
      for (const pseudo of ['::before', '::after']) {
        ...
        // `content: none/normal/""` set, which on this page skipped the one
        // pseudo-element that matters most: `.toggle-switch .slider::before` is
        // the round KNOB whose position is the entire non-colour indication of
        // whether "Use same nonce (attack scenario)" is on. Its host span is
        // `aria-hidden`, so both other oracles step over it too — it was
        // invisible to the whole gate while being the state cue for the control
        // that decides which half of this lab a reader sees.
        //
        ...
        // That is exact whenever the host paints one flat colour behind the
        // shape, which is the case here — the knob slides along a single-colour
        // track — and wrong for a shape that escapes its host, which is why
        // `escaped` is reported.
