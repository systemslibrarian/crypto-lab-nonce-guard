 ...
 *    they are exactly the surfaces that carry its verdicts: every
 *    `.status-badge` (`color-mix(in srgb, var(--status-*) 15%, transparent)`) —
 *    which is how every SAME NONCE USED / CONFIDENTIALITY BROKEN / FORGERY
 *    ACCEPTED result is painted — every `.chip` beside the two scheme headings,
 *    the `.probe-note` and `.threat-box` tints, `.chip-toggle.is-active`, and
 *    the `.cancel-term.shared` fill that the whole Section C lesson turns on.
 *    The hero aside is a `color-mix(in oklab, var(--accent) 6%, transparent)`
 *    and the shared top bar's `--cl-ink` is another. axe files all of them under
 *    `incomplete`, so a violations-only gate measured the contrast of almost
 *    nothing this lab actually says.
 ...
 *    at `.85`, `button.primary:disabled` at `.4`, and `.cancel-fadeable` inside
 *    a `.cancel-viz.is-cancelled`, which the Section C buttons drive to
 *    `opacity: 0` — a state this gate presses those buttons to reach.
 *
 ...
 *    boxes are `.demo-column` (`overflow: hidden`, which both live output panels
 *    sit inside), `.table-scroll` and `.equation` and `.cancel-viz` (all
 *    `overflow-x: auto`, and all three genuinely overflow at 380px). Content
 *    scrolled past those is not dimmed or partly drawn — it is absent from the
 ...
 *    draws its figures — the two `.flow-diagram`s and the two `.cancel-viz`
 *    algebra boards — in HTML and CSS flexbox rather than SVG, so the sibling
 *    walk finds nothing to do. Its only SVG is the shared top bar's two
 *    `aria-hidden` glyphs. The branch is kept because the alternative is a
 ...
 * not one — nor is it one to axe's `color-contrast` rule. This page has exactly
 * one pseudo-element that paints, and it carries real meaning:
 * `.toggle-switch .slider::before`, the round knob whose POSITION is the state
 * of the "Use same nonce (attack scenario)" switch. That gap is not left open
 * here: `nontext.ts` measures every `::before`/`::after` on the page against its
 * host's composited backdrop, and the gate asserts on the result, so the knob is
 * covered by an oracle rather than by a note.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks five sections of prose, two live output columns
     * whose contents are rebuilt on every run, and — the expensive part —
     * `#nonce-display`, which renders one `<span class="nonce-byte">` per byte
     * of the 96-bit nonce, plus a Level 2 attack panel of a dozen `.hex-output`
     * blocks. All of them re-walk the same ancestors up to `<body>`. Without the
     ...
     * that is `.demo-column` (`overflow: hidden`, wrapping both live output
     * columns), and `.table-scroll`, `.equation` and `.cancel-viz` — all
     * `overflow-x: auto`, and all three genuinely overflowing at 380px.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `visibility: hidden`. `.cancel-viz.is-cancelled .cancel-fadeable` sets
      // both `opacity: 0` and `visibility: hidden`, so once either Section C
      // button is pressed the cancelled `⊕` and the `keystream ⊕ keystream` /
      // `mask ⊕ mask` terms still have live geometry inside their board while
      // painting nothing. This page has no `<details>` at all — the gate this
      // replaces opened every one of them from script and there were none to
      // open, which is a fair summary of how much that step measured.
      ...
      // five elements carry it: `#msg1-desc`, `#msg2-desc` and `#siv-desc`
      // (`aria-describedby` targets), the comparison table's `<caption>`, and
      // `#demo-status` — a `role="status" aria-live="polite"` region whose text
      // is rewritten by every encrypt and every attack. Each has a rect and
      // passes `checkVisibility()` while painting exactly zero pixels;
      // `#demo-status` would report a fresh invented failure on every scan. The
     ...
     * or duplicated in exposed text:
     *  - the 🟢/🟡/🔴 dot that opens every `.status-badge`, whose words follow it
     *    in the same ink on the same fill, so measuring the badge measures the
     *    dot too;
     *  - the `↓` `.flow-arrow`s and `.siv-chain-arrow`s between diagram nodes;
     *  - the `.toggle-switch .slider` track, and `.cancel-rule`, both of which
     *    hold no text at all;
     *  - `.cancel-term.gone` ("= 0"), which is `display: none` until a
     *    `.cancel-viz` is cancelled and whose meaning is then also spoken by the
     *    `role="status"` caption underneath.
     * The two `.cancel-viz` boards and both `.flow-diagram`s carry `role="img"`
     * with a full `aria-label` rather than `aria-hidden`, so their terms stay
     * exposed and stay measured.
     *
     * Nothing on this page hides a VALUE. No nonce, ciphertext, tag, recovered
     * H, forged tag or verdict badge is inside an `aria-hidden` subtree — which
     * was checked rather than assumed, because that is the shared blind spot
     * where both oracles stop looking and it is the one place a live readout can
     ...
     *
     * This lab has no SVG of its own — every figure it draws is HTML and
     * flexbox. The guard stays because the usual way the mistake arrives is
     * exactly how this lab builds its DOM: `innerHTML` template literals, in
     * `renderEncryptOutput`, `doAttack` and `initSIVDemo`. Interpolating a
     * string ARRAY into one makes JS join it with commas, leaving a run of ","
     * text nodes inside whatever element wraps it — and inside a `<g>` those
     ...
     *
     * It is not a no-op here. `<html>` on this page has no background of its own
     * and `body { background: var(--background) }` supplies the canvas, so this
     * is the branch that resolves `#0f1117` (dark) / `#f4f5f7` (light) for the
     * whole document — five sections deep, well past the point where an
